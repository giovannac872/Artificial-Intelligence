...
class NODE:
    def __init__(self, state, cost, parent, positionNull, heuristic, f):
        self.state = state
        self.cost = cost
        self.parent = parent
...
def diferentTales(state, goalState):
    goal = [x for y in goalState for x in y]
    node = [x for y in state for x in y]
    increments=0
    #if all( goal[i] != node[i] for i in node):
    for i in goal:
        if goal[i] != node[i]:
            increments +=1
...
def rules(opens, closed, node):
    ...
    if auxiliar[0] > 0 and auxiliar[0]  <= 2: #primeira regra "subir posição nula"
        ...
        if avoidingRepeatStates(auxiliar2) != -1:
            ...
            F = diferentTales(auxiliar2, goalState) + (node.cost + 1)
            opens.append(NODE(auxiliar2, node.cost + 1, node, list(auxiliar), diferentTales(auxiliar2, goalState), F))
...
if __name__ == '__main__':
    ...
    closed = deque([])
    hashing = {}
    F = diferentTales(initialState, goalState) + 0 # profundidade 0, primeiro no
    opens.append(NODE(initialState, 0, None, indice(initialState), diferentTales(initialState, goalState), F))
    ...
    while aux == 1:
        if not opens:
            ...
            opens = sorted(opens, key = NODE.get_f)
            node = opens.pop(0)
            closed.append(node)
